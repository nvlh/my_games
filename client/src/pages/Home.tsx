...
import { ChevronDown, Download, Gamepad2, Info, Pencil, Play, Trash2, X } from "lucide-react";
...
import { buildPublicGameUpdateInput, getEditorView, isEditingPublicGame, shouldCloseEditorAfterSuccess, toPublicGameEditorForm } from "@/game/publicGameEditor";
...
export default function Home() {
  ...
  const [editingGameId, setEditingGameId] = useState<number>();
  ...
  const [gameForm, setGameForm] = useState({ name: "", slug: "", platform: "nes", genre: "action", description: "", players: "单人", input: "键盘/手柄", keySettings: "方向键/WASD；J/空格射击", fileLabel: "游戏文件", buttonLabel: "开始" });
  const active = useMemo(() => games.find((game) => game.mode === selected) ?? games[0], [selected]);
  const editorView = getEditorView(editingGameId);
  ...
  const editingPublicGame = editingGameId ? publicGames.data?.find((game) => game.id === editingGameId) : undefined;
  ...
  const controlHint = selectedPublicGame?.keySettings || "WASD / 方向键移动 · J / 空格射击";
  ...
  const createGame = trpc.publicGames.create.useMutation({ onSuccess: () => { setGameUploadError(""); setManagerOpen(false); setEditingGameId(undefined); setRomFile(undefined); setCoverFile(undefined); setIconFile(undefined); setScreenshotFile(undefined); setGameForm({ name: "", slug: "", platform: "nes", genre: "action", description: "", players: "单人", input: "键盘/手柄", keySettings: "方向键/WASD；J/空格射击", fileLabel: "游戏文件", buttonLabel: "开始" }); void trpcUtils.publicGames.list.invalidate(); }, onError: (error) => setGameUploadError(error.message) });
  const updateGame = trpc.publicGames.update.useMutation({ onSuccess: () => { setGameUploadError(""); if (shouldCloseEditorAfterSuccess(true)) { setManagerOpen(false); setEditingGameId(undefined); } setRomFile(undefined); setCoverFile(undefined); setIconFile(undefined); setScreenshotFile(undefined); void trpcUtils.publicGames.list.invalidate(); }, onError: (error) => setGameUploadError(error.message) });
  ...
  const editPublicGame = (game: NonNullable<typeof publicGames.data>[number]) => { setEditingGameId(game.id); setGameForm(toPublicGameEditorForm({ ...game, description: game.description ?? "" })); setRomFile(undefined); setCoverFile(undefined); setIconFile(undefined); setScreenshotFile(undefined); setGameUploadError(""); setManagerOpen(true); };
  ...
  const submitGameUpload = async () => {
    const editing = isEditingPublicGame(editingGameId);
    if (!editing && !romFile) { setGameUploadError("请先选择游戏文件"); return; }
    if (!gameForm.name.trim()) { setGameUploadError("请填写游戏名称"); return; }
    if (romFile) {
      const romExtension = romFile.name.split(".").pop()?.toLowerCase() ?? "";
      if (!( ["nes", "gba", "wsc", "iso", "cue", "bin", "cso", "pbp"] as string[]).includes(romExtension)) { setGameUploadError("游戏文件仅支持 NES、GBA、WSC、ISO、CUE、BIN、CSO 或 PBP"); return; }
    ...
    if (iconFile) {
      ...
      if (!( ["webp", "png", "jpg", "jpeg"] as string[]).includes(extension) || !["image/webp", "image/png", "image/jpeg"].includes(iconFile.type)) { setGameUploadError("游戏图标仅支持 WebP、PNG 或 JPEG"); return; }
      if (iconFile.size > 2 * 1024 * 1024) { setGameUploadError("游戏图标不能超过 2 MB"); return; }
    }
    const slug = gameForm.slug.trim() || gameForm.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-");
    try {
      const romPayloadBase64 = romFile ? await readFileBase64(romFile) : undefined;
      const coverPayloadBase64 = coverFile ? await readFileBase64(coverFile) : undefined;
      const iconPayloadBase64 = iconFile ? await readFileBase64(iconFile) : undefined;
      const screenshotPayloadBase64 = screenshotFile ? await readFileBase64(screenshotFile) : undefined;
      const common = { ...gameForm, platform: gameForm.platform as "xiaobawang" | "arcade" | "nes" | "gba" | "wsc" | "saturn" | "psp", genre: gameForm.genre as "action" | "shooter" | "platform" | "fighting" | "racing" | "puzzle" | "rpg" | "adventure" | "sports" | "other", slug };
      if (editingGameId) {
        const updateInput = buildPublicGameUpdateInput(editingGameId, { ...gameForm, slug }, {
          rom: romFile && romPayloadBase64 ? { name: romFile.name, contentType: romFile.type || "application/octet-stream", payloadBase64: romPayloadBase64 } : undefined,
          cover: coverFile && coverPayloadBase64 ? { name: coverFile.name, contentType: coverFile.type || "image/webp", payloadBase64: coverPayloadBase64 } : undefined,
          icon: iconFile && iconPayloadBase64 ? { name: iconFile.name, contentType: iconFile.type || "image/webp", payloadBase64: iconPayloadBase64 } : undefined,
          screenshot: screenshotFile && screenshotPayloadBase64 ? { name: screenshotFile.name, contentType: screenshotFile.type || "image/webp", payloadBase64: screenshotPayloadBase64 } : undefined,
        });
        updateGame.mutate(updateInput as Parameters<typeof updateGame.mutate>[0]);
      } else if (romFile && romPayloadBase64) {
        createGame.mutate({ ...common, romName: romFile.name, romContentType: romFile.type || "application/octet-stream", romPayloadBase64, coverName: coverFile?.name, coverContentType: coverFile?.type, coverPayloadBase64, iconName: iconFile?.name, iconContentType: iconFile?.type, iconPayloadBase64, screenshotName: screenshotFile?.name, screenshotContentType: screenshotFile?.type, screenshotPayloadBase64 });
      }
    ...
    <main className="arcade-shell">
      <section className="archive-main">
        <section className="library-console library-only" aria-label="个人 ROM 游戏库">
          ...
          <div className="platform-tabs">{platformLabels.map((item) => <button key={item.id} type="button" className={platform === item.id ? "active" : ""} onClick={() => setPlatform(item.id)}>{item.label}</button>)}<label className="import-button"><Play size={13} /> 导入合法 ROM<input type="file" accept=".nes,.gba,.wsc,.iso,.cso,.pbp,.cue,.bin" onChange={handleRomImport} /></label>{imported.length > 0 && <button type="button" onClick={() => setImported([])}>清空本地条目</button>}</div>{importError && <div className="import-error"><Info size={14} /> {importError}</div>}
          <div className="import-list" data-testid="game-library-list">{games.filter((game) => platform === "all" || platform === game.platform).map((game) => <div role="button" tabIndex={0} className={`import-row ${librarySelection === game.mode ? "is-selected" : ""}`} key={game.mode} onClick={() => { setSelected(game.mode); setLibrarySelection(game.mode); setPlaying(false); setSelectedSaveId(undefined); }} onKeyDown={(event) => { if (event.key === "Enter") startLibraryGame(game.mode, game.mode); }}><span className="file-led" /><span className="game-name-with-icon"><span className="game-icon-fallback" data-testid={`game-icon-${game.mode}`} aria-hidden="true"><Gamepad2 size={14} /></span><strong>{game.title}</strong></span><span>原创原型 · 单人</span><span>键盘/手柄</span><em>可运行</em><div className="row-actions" data-testid={`prototype-actions-${game.mode}`}><button type="button" disabled={!prototypeRowActions.canDownload} title="内置原创原型没有可下载 ROM" aria-label="内置原型不可下载"><Download size={12} /> 下载</button><button type="button" disabled={!prototypeRowActions.canDelete} title="内置原创原型不可删除" aria-label="内置原型不可删除"><Trash2 size={12} /> 删除</button><button type="button" className="row-start" data-testid={`start-${game.mode}`} onClick={(event) => { event.stopPropagation(); startLibraryGame(game.mode, game.mode); }}><Play size={12} /> 开始</button>{saveToolsFor(game.mode)}</div></div>)}{publicGames.data?.filter((game) => platform === "all" || game.platform === platform).map((game) => <div role="button" tabIndex={0} data-testid={`public-game-row-${game.slug}`} className={`import-row ${librarySelection === game.slug ? "is-selected" : ""}`} key={`public-${game.id}`} onClick={() => { setLibrarySelection(game.slug); setSelectedSaveId(undefined); setPlaying(false); }}><span className="file-led" /><span className="game-name-with-icon">{game.iconUrl ? <img className="game-icon" data-testid={`game-icon-${game.slug}`} src={game.iconUrl} alt="" loading="lazy" /> : <span className="game-icon-fallback" data-testid={`game-icon-${game.slug}`} aria-hidden="true"><Gamepad2 size={14} /></span>}<strong>{game.name}</strong></span><span>{game.platform === "nes" ? "小霸王 NES" : game.platform.toUpperCase()} · {game.genre} · {game.players}</span><span>{game.fileLabel || "游戏文件"} · {game.input}</span><em>服务器已收录</em><div className="row-actions" data-testid={`public-game-actions-${game.slug}`}><button type="button" data-testid={`public-game-edit-${game.slug}`} title="编辑游戏" onClick={(event) => { event.stopPropagation(); editPublicGame(game); }}><Pencil size={12} /> 编辑</button><a data-testid={`public-game-download-${game.slug}`} href={game.romUrl} download={game.romName} onClick={(event) => event.stopPropagation()}><Download size={12} /> 下载</a>{saveToolsFor(game.slug)}<button type="button" data-testid={`public-game-start-${game.slug}`} className="row-start" disabled={game.platform !== "nes"} title={game.platform === "nes" ? "开始游戏" : "该平台核心尚未接入"} onClick={(event) => { event.stopPropagation(); startLibraryGame(game.slug); }}><Play size={12} /> {game.buttonLabel || "开始"}</button><button type="button" data-testid={`public-game-delete-${game.slug}`} className="row-delete" title="删除游戏" onClick={(event) => { event.stopPropagation(); removePublicGame(game.id, game.name); }}><Trash2 size={12} /> 删除</button></div></div>)}{imported.filter((item) => platform === "all" || item.platform === platform).map((item) => <div role="button" tabIndex={0} data-testid={`imported-game-row-${item.name}`} className={`import-row ${librarySelection === item.name ? "is-selected" : ""}`} key={`${item.name}-${item.size}`} onClick={() => { setLibrarySelection(item.name); setSelectedSaveId(undefined); setPlaying(false); }}><span className="file-led" /><span className="game-name-with-icon"><span className="game-icon-fallback" data-testid={`game-icon-${item.name}`} aria-hidden="true"><Gamepad2 size={14} /></span><strong>{item.name}</strong></span><span>{item.platform === "nes" ? "小霸王 NES" : item.platform.toUpperCase()} · {item.genre} · {item.players}</span><span>{item.input}</span><em>{item.status}</em><div className="row-actions" data-testid={`imported-game-actions-${item.name}`}><button type="button" data-testid={`imported-game-download-${item.name}`} onClick={(event) => { event.stopPropagation(); downloadImportedGame(item.file); }}><Download size={12} /> 下载</button>{saveToolsFor(item.name)}<button type="button" data-testid={`imported-game-start-${item.name}`} className="row-start" disabled={item.platform !== "nes"} title={item.platform === "nes" ? "开始游戏" : "该平台核心尚未接入"} onClick={(event) => { event.stopPropagation(); startLibraryGame(item.name); }}><Play size={12} /> 开始</button><button type="button" data-testid={`imported-game-delete-${item.name}`} className="row-delete" title="删除本地游戏" onClick={(event) => { event.stopPropagation(); removeImportedGame(item.name); }}><Trash2 size={12} /> 删除</button></div></div>)}{games.filter((game) => platform === "all" || platform === game.platform).length === 0 && (!publicGames.data || publicGames.data.filter((game) => platform === "all" || game.platform === platform).length === 0) && imported.filter((item) => platform === "all" || item.platform === platform).length === 0 && <div className="empty-library"><Info size={16} /><span>当前平台还没有游戏。导入你拥有合法权利的 ROM，或切换到街机查看原创原型。</span></div>}{(games.length + (publicGames.data?.length ?? 0) + imported.length > 0) && !librarySelection && <div className="library-hint"><Info size={14} /> 请先点击一个游戏条目，开始按钮、存档下拉和下载/删除操作都在对应游戏后面。</div>}</div>
        </section>
      ...
      {managerOpen && <div className="rename-modal" data-testid="public-game-editor" role="dialog" aria-modal="true" aria-label={editingGameId ? "编辑公开游戏" : "公开游戏上传"}><div className="rename-card game-manager-card"><span className="section-index">PUBLIC GAME / {editingGameId ? "EDIT" : "UPLOAD"}</span><h3>{editingGameId ? "编辑游戏" : "上传游戏"}</h3><p className="manager-note">请只使用你拥有合法权利的游戏文件。编辑时 ROM、封面和截图均可留空，留空表示保留当前文件。</p><div className="manager-grid"><label>游戏名称<input data-testid="public-game-editor-name" value={gameForm.name} onChange={(event) => setGameForm({ ...gameForm, name: event.target.value })} placeholder="例如：我的街机游戏" /></label><label>英文标识<input value={gameForm.slug} onChange={(event) => setGameForm({ ...gameForm, slug: event.target.value })} placeholder="my-arcade-game" /></label><label>平台<select value={gameForm.platform} onChange={(event) => setGameForm({ ...gameForm, platform: event.target.value })}><option value="nes">小霸王 NES</option><option value="arcade">街机</option><option value="gba">GBA</option><option value="wsc">WSC / WonderSwan Color</option><option value="saturn">土星 SS</option><option value="psp">PSP</option></select></label><label>游戏类型<select value={gameForm.genre} onChange={(event) => setGameForm({ ...gameForm, genre: event.target.value })}><option value="action">动作</option><option value="shooter">射击</option><option value="platform">平台跳跃</option><option value="fighting">格斗</option><option value="racing">竞速</option><option value="puzzle">益智</option><option value="rpg">角色扮演</option><option value="adventure">冒险</option><option value="sports">体育</option><option value="other">其他</option></select></label><label>玩家数<input value={gameForm.players} onChange={(event) => setGameForm({ ...gameForm, players: event.target.value })} /></label><label>操控方式<input value={gameForm.input} onChange={(event) => setGameForm({ ...gameForm, input: event.target.value })} /></label><label>文件标签<input data-testid="public-game-editor-file-label" value={gameForm.fileLabel} onChange={(event) => setGameForm({ ...gameForm, fileLabel: event.target.value })} placeholder="游戏文件" /></label><label>按钮标签<input data-testid="public-game-editor-button-label" value={gameForm.buttonLabel} onChange={(event) => setGameForm({ ...gameForm, buttonLabel: event.target.value })} placeholder="开始" /></label></div><label className="manager-wide">简介<textarea value={gameForm.description} onChange={(event) => setGameForm({ ...gameForm, description: event.target.value })} rows={3} /></label><div className="manager-files">{editingPublicGame?.iconUrl && <div className="current-icon-preview" data-testid="public-game-editor-current-icon"><span>当前图标</span><img src={editingPublicGame.iconUrl} alt="当前游戏图标" /></div>}<label>{editingGameId ? "替换游戏文件（可选）" : "游戏文件"}<input type="file" accept=".nes,.gba,.wsc,.iso,.cue,.bin,.cso,.pbp" onChange={(event) => setRomFile(event.target.files?.[0])} /></label><label>游戏图标（可选，最大 2 MB）<input data-testid="public-game-editor-icon" type="file" accept="image/webp,image/png,image/jpeg" onChange={(event) => setIconFile(event.target.files?.[0])} /></label><label>封面图片<input type="file" accept="image/webp,image/png,image/jpeg" onChange={(event) => setCoverFile(event.target.files?.[0])} /></label><label>游戏截图<input type="file" accept="image/webp,image/png,image/jpeg" onChange={(event) => setScreenshotFile(event.target.files?.[0])} /></label></div><label className="manager-wide">按键设置<textarea data-testid="public-game-editor-key-settings" value={gameForm.keySettings} onChange={(event) => setGameForm({ ...gameForm, keySettings: event.target.value })} rows={2} placeholder="例如：方向键移动；J 键攻击；K 键跳跃" /></label>{gameUploadError && <div className="import-error"><Info size={14} /> {gameUploadError}</div>}<div><button type="button" onClick={() => { setManagerOpen(false); setEditingGameId(undefined); }}>取消</button><button type="button" data-testid="public-game-editor-submit" className="save-action" onClick={submitGameUpload} disabled={createGame.isPending || updateGame.isPending}>{createGame.isPending || updateGame.isPending ? "保存中" : editingGameId ? "保存修改" : "公开上传"}</button></div></div></div>}
