...
export default function Home() {
  ...
  const createGame = trpc.publicGames.create.useMutation({ onSuccess: () => { setGameUploadError(""); setManagerOpen(false); setEditingGameId(undefined); setRomFile(undefined); setCoverFile(undefined); setScreenshotFile(undefined); setGameForm({ name: "", slug: "", platform: "nes", genre: "action", description: "", players: "单人", input: "键盘/手柄" }); void trpcUtils.publicGames.list.invalidate(); }, onError: (error) => setGameUploadError(error.message) });
  const updateGame = trpc.publicGames.update.useMutation({ onSuccess: () => { setGameUploadError(""); if (shouldCloseEditorAfterSuccess(true)) { setManagerOpen(false); setEditingGameId(undefined); } setRomFile(undefined); setCoverFile(undefined); setScreenshotFile(undefined); void trpcUtils.publicGames.list.invalidate(); }, onError: (error) => setGameUploadError(error.message) });
  ...
  const editPublicGame = (game: NonNullable<typeof publicGames.data>[number]) => { setEditingGameId(game.id); setGameForm(toPublicGameEditorForm({ ...game, description: game.description ?? "" })); setRomFile(undefined); setCoverFile(undefined); setScreenshotFile(undefined); setGameUploadError(""); setManagerOpen(true); };
  ...
  const submitGameUpload = async () => {
    ...
    try {
      ...
      if (editingGameId) {
        ...
        createGame.mutate({ ...common, romName: romFile.name, romContentType: romFile.type || "application/octet-stream", romPayloadBase64, coverName: coverFile?.name, coverContentType: coverFile?.type, coverPayloadBase64, screenshotName: screenshotFile?.name, screenshotContentType: screenshotFile?.type, screenshotPayloadBase64 });
      }
